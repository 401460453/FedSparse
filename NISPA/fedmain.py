...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default="mnist",
                        choices=["mnist", "pmnist_partial", "fmnist", 'emnist', 'emnist_fmnist', 'cifar100', "cifar10"])
    ...
    parser.add_argument('--p_step_size', type=str, default="cosine",
                        choices=['cosine', 'exp_decay', 'linear'])  # how to determine p
    parser.add_argument('--step_size_param', type=float, default=30)  # k hyperparameter
    parser.add_argument('--grow_init', type=str, default="normal",
                        choices=['uniform', 'normal', 'zero'])  # initilization of new connections
    # full_random type1 + type 2 connections | flow_random type 2 connections | forwardT_random type 2 connections
    parser.add_argument('--rewire_algo', type=str, default="full_random",
                        choices=['full_random', 'forwardT_random', 'flow_random'])
    ...
    for task_id, taskset in enumerate(scenario_train):
        optimal_p, percentange_of_frozen_conns, acc_after_fine_tuning, _, list_of_ps, num_important_units = learner.task_train(
            task_id, taskset, None)
