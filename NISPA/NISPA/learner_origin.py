        ...
        while (True):
            ...
            if (acc_new < (acc_prev - self.args.recovery_perc) and t > 2) or stable_perc <= 5:
                ...
                if len(self.stable_units_set) == 0:
                    ...
                    self.stable_units_set = [prev_set.union(set(stable_index)) for stable_index, prev_set in
                                             zip(stable_indices_prev, self.stable_units_set)]
                    self.stable_indices = [list(stable_set) for stable_set in self.stable_units_set]
            ...
            if self.args.grow:
                ...
                num_dropped_connections = [torch.sum(torch.abs(a - b)).cpu().numpy() for a, b in
                                           zip(connectivity_before, connectivity_after)]
                plastic_units = compute_plastic(current_model, stable_indices)
                # Fix Non importants without outgoing connections
                current_model, remainder_connections = fix_no_outgoing(current_model, plastic_units,
                                                                       num_dropped_connections)
                weights = list_of_weights(current_model)

                activations, _ = compute_average_activation(current_model, train_loader, DEVICE, current_classes,
                                                            False)  # 这里计算activation干嘛？？？
                current_model, remainder_connections = grow_connection_A2B(current_model, remainder_connections,
                                                                           [weights[0][0].shape[1]] + [w.shape[0] for
                                                                                                       w, _ in weights],
